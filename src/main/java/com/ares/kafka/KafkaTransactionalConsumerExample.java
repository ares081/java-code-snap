...
public class KafkaTransactionalConsumerExample {
  ...
  public static void main(String[] args) {
    ...
    props.put(ConsumerConfig.GROUP_ID_CONFIG, "ts-commit-example");
    ...
    try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props)) {
      consumer.subscribe(List.of("T-transaction-example"));
      while (true) {
        ...
        for (ConsumerRecord<String, String> record : records) {
          ...
          log.info(record.key());
          record.headers().forEach(e -> log.info("key:{}-> value:{}", e.key(), e.value()));
        }
      }
    }
  }
